...
def test_frozen_config_and_constants():
    ...
    assert c["experiment_version"] == "regional-recompute-round0-v4.1" and c["attempt"] == 2 and c["attempt1"]["decision"] == "INVALID_EXPERIMENT" and r0.NAMESPACE == "regional_recompute_round0" and r0.ATTEMPT_SUBDIR == "rev4_1"
    assert "diagnostic-only" in c["rev4_validation_note"] and "No scientific thresholds" in c["rev4_validation_note"]
...
def test_output_namespace_isolation():
    r0.validate_output_path(r0.REPO_ROOT / "results" / r0.NAMESPACE / "rev4_1")
    r0.validate_output_path(r0.REPO_ROOT / "results" / r0.NAMESPACE / "rev4_1" / "sub")
    for bad in (r0.REPO_ROOT / "results" / "video_trajectory_fork_confirmatory", r0.REPO_ROOT / "results" / "speculative_contamination_round0", r0.REPO_ROOT / "results" / r0.NAMESPACE, r0.REPO_ROOT / "results" / r0.NAMESPACE / "attempt3", r0.REPO_ROOT / "results" / "other" / "rev4_1"):
        with pytest.raises(ValueError):
...
def test_velocity_validation_is_robust_to_bf16_quantisation():
    ...
    assert ok["pass"] and ok["scheduler_output_bit_exact"] and ok["single_step_bit_exact"] and ok["bf16_reconstruction_exact_fraction"] >= 0.999
    assert set(ok["diagnostic_only"]) >= {"delta_sigma_ls_slope", "delta_sigma_relative_error", "sign_agreement"} and ok["diagnostic_only"]["sign_agreement"]
    ...
    # the slope is reported but can never gate: even a wildly wrong slope leaves pass unchanged when the forward checks hold
    assert "delta_sigma_relative_error" not in ok7 and ok7["diagnostic_only"]["delta_sigma_relative_error"] < 0.15
    # single-step output mismatch now gates
    assert not r0.velocity_validation(x_k, x_k1, within, x_k, sig, k)["pass"]
...
def test_preregistration_seal_and_gpu_refusal(tmp_path, monkeypatch):
    ...
    out = r0.REPO_ROOT / "results" / r0.NAMESPACE / "rev4_1" / "_pytest_tmp"
    ...
    try:
        ...
        assert prereg["attempt"] == 2 and set(prereg["prior_runs"]) == {"attempt1_rev3", "rev4_reference_validation"} and "probe-captured" in prereg["error_models"]["STALE_VELOCITY"]["formula"]
        assert "REPORT ONLY" in prereg["scheduler_algebra"]["on_host_validation"] and "diagnostic-only" in prereg["validation_provenance_note"]
