"""CPU contracts for the Round 4C-0 action edit, attempt 2. No GPU."""
...
from experiments import video_trajectory_fork_action_edit_attempt2_killtest as ae

pytestmark = [pytest.mark.core_model, pytest.mark.cpu, pytest.mark.diffusion]
CONFIG_PATH = Path("experiments/video_trajectory_fork_action_edit_attempt2_killtest_config.yaml")
...
def test_frozen_design():
    ...
    assert "static camera" not in ae.OLD_PROMPT and "static camera" not in ae.NEW_PROMPT
    assert ae.OLD_PROMPT == ae.me.OLD_PROMPT == "A red sports car driving on a snowy road, cinematic video" and ae.NEW_PROMPT == "A red sports car parked on a snowy road, cinematic video"
    assert config["attempt"] == 2 and config["no_third_attempt"] is True and "No third attempt" in config["gate"]["final_decision_rule"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda c: c["prompts"].__setitem__("new", "A red sports car parked motionless on a snowy road, static camera, cinematic video"),
        lambda c: c["prompts"].__setitem__("old", "A red sports car driving forward on a snowy road, static camera, cinematic video"),
        lambda c: c["prompts"].__setitem__("new", "A red sports car drifting on a snowy road, cinematic video"),
        lambda c: c.__setitem__("attempt", 3),
        lambda c: c.__setitem__("no_third_attempt", False),
...
def test_output_namespace_isolation():
    ...
    for bad in ("video_trajectory_fork_action_edit_killtest", "video_trajectory_fork_multi_edit_killtest", "video_trajectory_fork_oracle_gap_killtest", "video_trajectory_fork_confirmatory", "other"):
        with pytest.raises(ValueError):
...
def test_final_decision_rule():
    ...
    assert d["decision"] == "STOP_DIFFUSION_REUSE" and d["BRANCH_CLOSED"] is True  # second baseline failure closes the branch
...
@pytest.mark.skipif(not (ae.ROUND4C0_DIR / "summary.json").exists() or not (ae.ATTEMPT1_DIR / "summary.json").exists(), reason="Round 4C-0 / attempt-1 results not present")
def test_preregistration_immutability(tmp_path, monkeypatch):
    ...
    try:
        ...
        assert prereg["baseline_informativeness"]["method"] == "blinded_human" and "INVEST_4C1 iff action" in prereg["final_decision_rule"] and prereg["attempt"] == 2 and prereg["no_third_attempt"] is True
        a1 = prereg["attempt1_reference"]
        assert a1["decision"] == "INVALID" and a1["informative_seeds"] == [5678, 6789] and a1["baseline_labels"]["7890"]["old"] == "NEW"
        assert set(prereg["round4c0_old_baseline_hashes"]) == {str(x) for x in ae.SEEDS}
