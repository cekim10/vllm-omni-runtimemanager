#!/usr/bin/env python3
"""Round 4C-0 action edit, ATTEMPT 2 (final; no third attempt) - STOP / INVEST decision for the fork branch.

Attempt 1 (results/video_trajectory_fork_action_edit_killtest) was INVALID: the OLD prompt "driving forward,
static camera" produced stationary cars for seeds 7890 and 8901 (blind labels NEW / MIXED_NEW_DOMINANT), leaving
2 informative seeds. Attempt 2 repairs only the baseline: OLD returns to the Round 4C-0 known-good prompt whose
trajectories are already known to drive (motion energy 0.023-0.041 on the same seeds), NEW becomes "parked", and
"static camera" is removed from both. Everything else is the attempt-1 protocol unchanged: seeds, K grid,
control K, success set, dispersion, gap_retry, STRONG/WEAK rule, minimum informative seeds, blinded-human
informativeness, descriptive-only motion metric.

Controls: C0 - the OLD baselines must be bit-exact with the Round 4C-0 OLD baselines (same prompt/seed/config),
proving the repair returned exactly to the known-good baseline; C1 OLD->OLD exactness at K=8.

Final rule (frozen): action EDIT_STRONG -> INVEST_4C1; valid but not STRONG -> STOP_DIFFUSION_REUSE;
INVALID again -> STOP_DIFFUSION_REUSE (a second baseline failure ends the branch). No third attempt.
"""
...
EXPERIMENT_VERSION = "video-trajectory-fork-action-edit-attempt2-v1"
NAMESPACE = "video_trajectory_fork_action_edit_attempt2_killtest"
ATTEMPT1_NAMESPACE = "video_trajectory_fork_action_edit_killtest"
ATTEMPT1_DIR = REPO_ROOT / "results" / ATTEMPT1_NAMESPACE
DEFAULT_CONFIG = REPO_ROOT / "experiments/video_trajectory_fork_action_edit_attempt2_killtest_config.yaml"
...
TOTAL_STEPS = 40
OLD_PROMPT = conf.OLD_PROMPT  # Round 4C-0 known-good driving prompt: "A red sports car driving on a snowy road, cinematic video"
NEW_PROMPT = "A red sports car parked on a snowy road, cinematic video"
assert OLD_PROMPT == me.OLD_PROMPT
...
TRUSTED_SOURCE_FILES = (
    "experiments/video_trajectory_fork_action_edit_attempt2_killtest.py",
    "experiments/video_trajectory_fork_action_edit_attempt2_killtest_config.yaml",
    "experiments/run_video_trajectory_fork_action_edit_attempt2_killtest_gpu0.sh",
    "tests/diffusion/test_video_trajectory_fork_action_edit_attempt2_killtest.py",
...
FORBIDDEN_OUTPUT_PARTS = me.FORBIDDEN_OUTPUT_PARTS + (me.NAMESPACE, ATTEMPT1_NAMESPACE)
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if "static camera" in prompts["old"] or "static camera" in prompts["new"]:
        raise ValueError("Attempt 2 removes the static-camera phrase from both prompts")
    if int(config["attempt"]) != 2 or config["no_third_attempt"] is not True:
        raise ValueError("This is attempt 2 and the last one")
    if config["edit"] != EDIT:
...
def family(seed: int) -> dict[str, Any]:
    return {"id": f"{EDIT}_seed{int(seed)}", "severity": "round4c_action_attempt2", "seed": int(seed), "edit": EDIT, "old_prompt": OLD_PROMPT, "new_prompt": NEW_PROMPT, "old_concept": OLD_CONCEPT, "new_concept": NEW_CONCEPT}
...
def attempt1_reference() -> dict[str, Any]:
    """Frozen attempt-1 pins; attempt 2 is only allowed after an INVALID attempt 1."""
    for name in ("preregistration.sha256", "summary.json"):
        if not (ATTEMPT1_DIR / name).exists():
            raise GateError(f"Attempt-1 reference missing: {ATTEMPT1_DIR / name}")
    summary = json.loads((ATTEMPT1_DIR / "summary.json").read_text())
    if summary.get("decision") != "INVALID":
        raise GateError(f"Attempt-1 decision is {summary.get('decision')!r}; a second attempt is only allowed after INVALID")
    return {
        "namespace": ATTEMPT1_NAMESPACE,
        "preregistration_sha256": (ATTEMPT1_DIR / "preregistration.sha256").read_text().split()[0],
        "summary_sha256": sha256_file(ATTEMPT1_DIR / "summary.json"),
        "decision": summary["decision"],
        "informative_seeds": summary["informative_seeds"],
        "baseline_labels": {s: {"old": v["old_baseline_label"], "new": v["new_baseline_label"]} for s, v in summary["baseline_informativeness"]["seeds"].items()},
        "informative_k_star": summary["edit"]["k_star_by_seed"],
        "failure_cause": "OLD prompt 'driving forward, static camera' produced stationary cars for 2 of 4 seeds",
    }


def round4c0_old_baseline_hashes() -> dict[str, Any]:
    out = {}
    for seed in SEEDS:
        row = json.loads((ROUND4C0_DIR / "rows" / f"old_seed{seed}" / "old_baseline.json").read_text())
        if row["active_prompt"] != OLD_PROMPT:
            raise GateError("Round 4C-0 OLD prompt differs from the attempt-2 OLD prompt; C0 reference invalid")
        out[str(seed)] = {"final_latent_hash": row["final_latent_hash"], "video_hash": row["video_hash"], "initial_latent_hash": row["initial_latent_hash"]}
    return out
...
def build_preregistration(config: dict[str, Any], provenance: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "title": "Round 4C-0 action edit, attempt 2 (final; no third attempt)",
        "question": "For the action edit (driving -> parked) on the same four seeds, do per-trajectory K* disperse and does the trajectory oracle beat the best detect-and-retry static depth by >= 0.15 of a generation?",
        ...
        "round4c0_old_baseline_hashes": round4c0_old_baseline_hashes(),
...
def run_baselines(config: dict[str, Any], config_path: Path, output_dir: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        for seed in SEEDS:
            ...
            ref = prereg["round4c0_old_baseline_hashes"][str(seed)]
            identity[str(seed)] = {"identical_initial_latent": old_initial == new_initial, "initial_latent_hash": old_initial,
                                   "c0_old_bit_exact_vs_round4c0": old_row["final_latent_hash"] == ref["final_latent_hash"] and old_row["video_hash"] == ref["video_hash"] and old_initial == ref["initial_latent_hash"],
    ...
    return {"mode": "baselines", "rows": len(rows), "identity": {s: v["identical_initial_latent"] for s, v in identity.items()}, "c0_bit_exact": {s: v["c0_old_bit_exact_vs_round4c0"] for s, v in identity.items()}, "next": "run controls (no manual baseline judgment; baselines are labelled blind later)"}
...
def run_controls(config: dict[str, Any], config_path: Path, output_dir: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    c0_failures = [s for s, v in evidence["seeds"].items() if not v["c0_old_bit_exact_vs_round4c0"]]
    preflight: dict[str, Any] = {"provenance_hash": provenance["provenance_hash"], "identical_initial_latent": {s: v["identical_initial_latent"] for s, v in evidence["seeds"].items()}, "c0_failures": c0_failures, "seeds": {}}
    if c0_failures:
        preflight["status"] = "INVALID_C0"
        atomic_json(output_dir / "preflight.json", preflight)
        raise GateError(f"C0 failed: OLD baselines not bit-exact with Round 4C-0 for seeds {c0_failures}")
    omni = smoke._build_omni(config, args)
    rows: list[dict[str, Any]] = []
    try:
        for seed in SEEDS:
...
def require_controls(output_dir: Path, provenance_hash: str) -> dict[str, Any]:
    ...
    if preflight.get("provenance_hash") != provenance_hash or preflight.get("status") != "PASS" or set(preflight["seeds"]) != {str(s) for s in SEEDS} or preflight.get("c0_failures"):
...
def final_decision(action_status: str, round4c0_status: dict[str, str]) -> dict[str, Any]:
    ...
    if action_status == "EDIT_INVALID":
        return {"decision": "STOP_DIFFUSION_REUSE", "rationale": f"action edit INVALID for the second time; 4C-0 statuses {round4c0_status}; a second baseline failure closes the branch (frozen attempt-2 rule)", "BRANCH_CLOSED": True, "strong_edits": strong}
...
def run_analyze(config: dict[str, Any], config_path: Path, output_dir: Path) -> dict[str, Any]:
    ...
    summary = {
        **verdict, "action_status": status, "round4c0_edit_status": prereg["round4c0_reference"]["edit_status"], "edit": {**gap, "status": status, "label_matrix": {str(s): {str(k): fork_labels[s][k] for k in KS} for s in informative},
        ...
        "controls": {"preflight_status": preflight["status"], "c0_failures": preflight.get("c0_failures", [])}, "attempt": 2, "attempt1_reference": prereg["attempt1_reference"], "trajectories_executed": len(rows),
...
def write_report(output_dir: Path, s: dict[str, Any]) -> None:
    e = s["edit"]
    L = ["# Round 4C-0 action edit, attempt 2 (final; no third attempt)", "", "## Decision", "", s["decision"], "", s["rationale"], "", "Attempt 1 was INVALID (OLD baseline prompt produced stationary cars for 2 of 4 seeds); attempt 2 repaired only the baseline prompt and removed 'static camera'. C0 ties the OLD trajectories bit-exactly to Round 4C-0.", "",
         ...
         f"- EDIT_STRONG iff DISPERSION and gap_retry >= {STRONG_MIN_GAP}; EDIT_INVALID below {MIN_INFORMATIVE_SEEDS} informative seeds or C1 failure.", "- Final: INVEST_4C1 iff action EDIT_STRONG; valid but not STRONG -> STOP_DIFFUSION_REUSE; INVALID again -> STOP_DIFFUSION_REUSE. No third attempt.",
    ...
    (output_dir / "video_trajectory_fork_action_edit_attempt2_killtest.md").write_text("\n".join(L))
