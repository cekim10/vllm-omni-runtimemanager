#!/usr/bin/env bash
# Numerical Compatibility Round 0 (rev 0.2f). GPU 0 only. Execution configuration is chosen by run.py from --axis;
...
if [ $# -lt 1 ]; then
  ...
  echo "  AXIS in {EAGER,OFFLOAD,ATTN_SDPA,ATTN_FLASHINFER,ATTN_CUDNN}; CONFIG in {CANONICAL,EAGER,OFFLOAD,ATTN_SDPA,ATTN_FLASHINFER,ATTN_CUDNN}" >&2
  exit 2
fi
PHASE="$1"; shift
AXIS_ARGS=()
...
OUT="${1:-results/numerical_compatibility_round0/rev0_3}"
