...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ablation 1: fidelity policy memory-quality frontier.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=1 << 30,
        help="State manager CPU tier budget. Defaults to 1 GiB to avoid placement confounds.",
    )
    parser.add_argument("--theta-h", type=float, default=0.7, help="LOSSLESS threshold for value-aware mode.")
    parser.add_argument("--theta-w", type=float, default=0.3, help="COMPRESSED threshold for value-aware mode.")
...
async def _run_condition(
    ...
    *,
    ...
    request_id = f"{fidelity_mode}-p{prompt_id}-r{run_idx}-s{failure_step_idx}"
    with _fidelity_mode_override(engine, fidelity_mode):
        start = time.perf_counter()
        run_task = asyncio.create_task(
            engine.async_add_req_and_wait_for_response(_make_request(request_id, prompt, sampling_params))
        )
        ...
        if not aborted_output.aborted:
            raise RuntimeError(f"Expected an aborted request for {request_id}, got: {aborted_output!r}")

        restore_start = time.perf_counter()
        resumed_request = engine.restore_request_from_state(
            _make_request(request_id, prompt, sampling_params),
            request_id=request_id,
        )
    ...
    image_path = (
        ...
        / fidelity_mode
        / f"resume_f{failure_step_idx}_r{run_idx}.png"
    )
    _save_image(image, image_path)
    return (
        {
        ...
        aggregate.update(
            {
                ...
                "mean_checkpoint_size_bytes": mean(float(row["checkpoint_size_bytes"]) for row in group_rows),
            ...
            for failure_step_idx, frac in failure_points:
                for fidelity_mode in args.fidelity_modes:
                    for run_idx in range(args.runs_per_condition):
                        ...
                        metrics = _image_metrics(resumed_image, baseline.image)
                        csv_row = {
                            "model": args.model,
                            "prompt_id": prompt_id,
                            "prompt": prompt_text,
                            "run_idx": run_idx,
                            "fidelity_mode": fidelity_mode,
                            "assigned_fidelity": result["assigned_fidelity"],
                            "placement": result["placement"],
                            "failure_step_frac": frac,
                            "failure_step_idx": result["failure_step_idx"],
                            "total_steps": args.num_inference_steps,
                            "checkpoint_size_bytes": result["checkpoint_size_bytes"],
                            "checkpoint_value_score": result["checkpoint_value_score"],
                            "ttfv_sec": result["ttfv_sec"],
